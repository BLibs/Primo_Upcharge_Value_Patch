...
from get_data import fetch_data_in_parallel, get_est_count
...
if __name__ == "__main__":
    # Establishment parameters for looping
    MIN_EST = 3  # Test establishment
    MAX_EST = 3  # Setting max to 3 as well for testing
    # MAX_EST = get_est_count() # Turned off for testing. Would run on all 121 establishments if active

    # Get the product data for all selected establishments. ***Running against all est by default. Variable in config***
    all_product_data = fetch_data_in_parallel(MIN_EST, MAX_EST)
